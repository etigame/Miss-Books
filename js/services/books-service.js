...
function save(book) { // support situation of update or create new
  if (book.id) {
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    books = [ //I can also import file gBooks (with all this data) and write: books = gBooks
      {
        id: 'OXeMG8wNskc',
...
function addReview(bookId, review) {
  ...
  return storageService.get(BOOKS_KEY, bookId) // get the book
  .then(book => {
    if (!book.reviews) book.reviews = []
    book.reviews.unshift(review)

    return storageService.put(BOOKS_KEY, book) // update the book + return because "then"=promise
  })
}
